...
function filterSaved(all, startDate, endDate, dir = '', metric = '') {
  ...
  return all.filter(x => {
    ...
    const running = worked || RUNNING_WITH_KAVACH_REASONS.has(reason) || /OTHER MAKE/.test(reason);
    if (targetMetric === 'runningWithKavach' || targetMetric === 'fitted') return running;
    if (targetMetric === 'working') return worked;
    if (targetMetric === 'defective') return ['KERNEX APP. PENDING','KAVACH DO NOT START','KAVACH ISOLATION PREV.'].includes(reason);
    if (targetMetric === 'kernex') return reason === 'KERNEX APP. PENDING';
    if (targetMetric === 'doNotStart') return reason === 'KAVACH DO NOT START';
    if (targetMetric === 'isolation') return reason === 'KAVACH ISOLATION PREV.';
    if (targetMetric === 'other') return reason === 'OTHER';
    if (targetMetric === 'otherMake') return /OTHER MAKE/.test(reason) || /OTHER MAKE/.test(String(x.row?.kavachMake || '').toUpperCase());
    if (targetMetric === 'total') return DEFECTIVE_REASONS.has(reason) || reason === 'OTHER';
    if (targetMetric === 'nonKavach') return Object.values(NON_KAVACH_CATEGORIES).includes(reason);
    if (targetMetric === 'nonKavachLink') return reason === NON_KAVACH_CATEGORIES.LINK;
    if (targetMetric === 'nonKavachAttach') return reason === NON_KAVACH_CATEGORIES.ATTACH;
    if (targetMetric === 'nonKavachPlain') return reason === NON_KAVACH_CATEGORIES.PLAIN;
    return true;
  });
}
...
function summaryForRecords(records, date, dir) {
  ...
  // Keep the Summary's NO. OF LOCO KAVACH FITTED definition explicit. It is
  // the same saved-record classification used by the Summary & Detail
  // running-with-Kavach metric: working + KAVACH DEFECTIVE + OTHER MAKE/OTHER.
  // Do not derive this from a different date query or workbook.
  const otherMake = records.filter(x => /OTHER MAKE/.test(savedReason(x)) || /OTHER MAKE/.test(String(x.row?.kavachMake || '').toUpperCase())).length;
  const fitted = working + defective + otherMake + other;
  const runningWithKavach = fitted;
  return {
    day, date, dir, totalTrains: records.length, fitted, runningWithKavach, working, defective, kernex, doNotStart, isolation, other, total, otherMake,
...
function detailSummaryForRecords(records) {
  ...
  const category = NON_KAVACH_CATEGORIES;
  const worked = x => String(x.row?.worked || '').trim().toUpperCase() === 'YES';
  // Details item 5 follows the explicit three-reason rule from the latest
  // requirements. The running-with-Kavach metric remains the report's
  // existing KAVACH DEFECTIVE + OTHER definition.
  const defective = x => DETAIL_KAVACH_DEFECTIVE_REASONS.has(reason(x));
  const runningDefective = x => reason(x) === 'KAVACH DEFECTIVE';
  ...
  const nonKavach = x => Object.values(category).includes(reason(x));
  const section = arr => {
    const workingCondition = arr.filter(worked).length;
    const defectiveCount = arr.filter(defective).length;
    const runningDefectiveCount = arr.filter(runningDefective).length;
    const otherMakeCount = arr.filter(otherMake).length;
    const otherReasonCount = arr.filter(otherReason).length;
    const nonKavachCount = arr.filter(nonKavach).length;
    const runningWithKavach = workingCondition + runningDefectiveCount + otherMakeCount + otherReasonCount;
    return { targetTrains: arr.length, runningWithKavach, workingCondition, nonKavach: nonKavachCount, defective: defectiveCount, otherMake: otherMakeCount, otherReason: otherReasonCount,
      nonKavachLink: arr.filter(x => reason(x) === category.LINK).length,
      nonKavachAttach: arr.filter(x => reason(x) === category.ATTACH).length,
      nonKavachPlain: arr.filter(x => reason(x) === category.PLAIN).length };
  };
  ...
  const divisionRows = (getDivision, wantedReason) => {
    const map = new Map();
    records.filter(x => reason(x) === wantedReason).forEach(x => {
      const value = getDivision(x); if (!value) return;
      if(!map.has(value)) map.set(value,{name:value,total:0,up:0,dn:0});
      const item=map.get(value); item.total++; if(savedDir(x)==='UP')item.up++; if(savedDir(x)==='DN')item.dn++;
    });
  ...
  records.filter(defective).forEach(x=>{const name=String(x.row?.shed||'—').trim()||'—';if(!shedMap.has(name))shedMap.set(name,{name,total:0,up:0,dn:0});const item=shedMap.get(name);item.total++;if(savedDir(x)==='UP')item.up++;if(savedDir(x)==='DN')item.dn++;});
  const nonKavachPlain = divisionRows(x=>savedValue(x,['attachedDiv','LOCO_ATTACHED_DIV.','LOCO_ATTACHED_DIV','LOCO_ATTACHED_DIVISION']),category.PLAIN).map(r=>({...r,station:r.name}));
  return {
    total:section(records),up:section(up),dn:section(dn),defectiveRows,defectiveByShed:[...shedMap.values()].sort((a,b)=>b.total-a.total),
    nonKavachLinkDivision:divisionRows(x=>savedValue(x,['locoLinkDiv','LOCO_LINK_DIV','LOCO_LINK_DIVISION','Loco Link Division']),category.LINK),
    nonKavachAttachedDivision:divisionRows(x=>savedValue(x,['attachedDiv','LOCO_ATTACHED_DIV.','LOCO_ATTACHED_DIV','LOCO_ATTACHED_DIVISION','Loco Attached Division']),category.ATTACH),
    nonKavachPlain,otherRows
  };
}
...
function detailMetricRecords(records, metric, dir='', groupValue='') {
  ...
  const worked = x => String(x.row?.worked||'').trim().toUpperCase() === 'YES';
  switch (metric) {
    case 'targetTrains': break;
    case 'runningWithKavach': case 'fitted': rows = rows.filter(x => worked(x) || reason(x)==='KAVACH DEFECTIVE' || reason(x)==='OTHER' || /OTHER MAKE/.test(reason(x)) || /OTHER MAKE/.test(String(x.row?.kavachMake||'').toUpperCase())); break;
    case 'working': rows = rows.filter(worked); break;
    case 'defective': rows = rows.filter(x => DETAIL_KAVACH_DEFECTIVE_REASONS.has(reason(x))); break;
    case 'kernex': rows = rows.filter(x => reason(x)==='KERNEX APP. PENDING'); break;
    case 'doNotStart': rows = rows.filter(x => reason(x)==='KAVACH DO NOT START'); break;
    case 'isolation': rows = rows.filter(x => reason(x)==='KAVACH ISOLATION PREV.'); break;
    case 'other': case 'otherReason': rows = rows.filter(x => reason(x)==='OTHER'); break;
    case 'otherMake': rows = rows.filter(x => /OTHER MAKE/.test(reason(x)) || /OTHER MAKE/.test(String(x.row?.kavachMake||'').toUpperCase())); break;
    case 'nonKavach': rows = rows.filter(x => Object.values(NON_KAVACH_CATEGORIES).includes(reason(x))); break;
    case 'nonKavachLink': rows = rows.filter(x => reason(x)===NON_KAVACH_CATEGORIES.LINK); break;
    case 'nonKavachAttach': rows = rows.filter(x => reason(x)===NON_KAVACH_CATEGORIES.ATTACH); break;
    case 'nonKavachPlain': rows = rows.filter(x => reason(x)===NON_KAVACH_CATEGORIES.PLAIN && (!groupValue || String(x.row?.attachedDiv||'').trim()===String(groupValue).trim())); break;
    case 'total': rows = rows.filter(x => DEFECTIVE_REASONS.has(reason(x)) || reason(x)==='OTHER'); break;
    case 'defectiveByShed': rows = rows.filter(x => DETAIL_KAVACH_DEFECTIVE_REASONS.has(reason(x)) && String(x.row?.shed||'').trim()===String(groupValue||'').trim()); break;
    default: break;
  }
  return rows;
}
...
function buildPdfDocument(pages) {
  ...
  for(const page of pages){
    ...
    const pageId=add(`<< /Type /Page /Parent PAGES /MediaBox [0 0 595.28 841.89] /Resources << /Font << /F1 ${fontId} 0 R >> >> /Contents ${contentId} 0 R >>`); pageIds.push(pageId);
  }
...
function createSummaryDetailPdf(report){
  const W=595.28,H=841.89,M=28.35, contentW=W-M*2, pages=[], ops=[];
  const headerH=44, footerH=20, bottomLimit=M+footerH+6;
  let y=H-M-headerH, pageNo=1;

  const text=(x,yy,str,size=7,bold=false)=>{
    ops.push(`BT /F1 ${size} Tf ${x.toFixed(2)} ${yy.toFixed(2)} Td (${pdfEscape(pdfAscii(str))}) Tj ET`);
  };
  ...
  const drawPageHeader=()=>{
    rectFill(M,H-M-31,contentW,31,0.93);
    text(M+10,H-M-14,'TRACTION OPERATION - KAVACH MONITORING SYSTEM',10,true);
    text(M+10,H-M-26,'West Central Railway, Kota Division',7,false);
    text(W-M-10,H-M-14,`Summary & Detail | ${report.fromDate} to ${report.toDate}`,6.5,false);
    line(M,H-M-33,W-M,H-M-33);
  };
  const drawPageFooter=()=>{
    line(M,M+12,W-M,M+12);
    text(M,M+3,'Traction Operation - Kavach Monitoring System | Kota Division',6,false);
    text(W-M-105,M+3,`Page ${pageNo}`,6,false);
  };
  ...
  const wrapFor=(value,width,fs,pad=3)=>wrapPdfText(value,Math.max(3,Math.floor((width-pad*2)/(fs*0.52))));
  const rowHeight=(cells,widths,fs,pad=3,lineH=7.6)=>{
    const wraps=cells.map((c,i)=>wrapFor(c,widths[i],fs,pad));
    return Math.max(1,...wraps.map(a=>a.length))*lineH+pad*2;
  };

  const table=(headers, rows, widths, opts={})=>{
    const fs=opts.fontSize||6.5, pad=opts.pad||3, lineH=opts.lineH||7.6;
    let rowIndex=0;
    const drawRow=(cells,isHeader=false)=>{
      const wraps=cells.map((c,i)=>wrapFor(c,widths[i],fs,pad));
      const rh=Math.max(1,...wraps.map(a=>a.length))*lineH+pad*2;
      if(y-rh<bottomLimit){
        newPage();
        if(!isHeader) drawRow(headers,true);
      }
      let x=M;
      const rowBg=isHeader ? 0.90 : (rowIndex%2===0 ? 0.985 : 1);
      if(rowBg<1) rectFill(M,y-rh,widths.reduce((a,b)=>a+b,0),rh,rowBg);
      for(let i=0;i<widths.length;i++){
        line(x,y,x,y-rh); line(x+widths[i],y,x+widths[i],y-rh);
        const center=opts.alignCenter && (opts.alignAll || i>0);
        for(let j=0;j<wraps[i].length;j++){
          const tx=center ? x+widths[i]/2-(wraps[i][j].length*fs*0.24) : x+pad;
          text(tx,y-pad-fs-j*lineH,wraps[i][j],fs,isHeader);
        }
        x+=widths[i];
      }
      line(M,y-rh,M+widths.reduce((a,b)=>a+b,0),y-rh);
      y-=rh;
  ...
  const section=(title,headers,rows,widths,opts={})=>{
    const fs=opts.fontSize||6.5;
    const first=rows.length?rows[0]:headers.map(()=> '');
    const titleH=20, gap=5, needed=titleH+gap+rowHeight(headers,widths,fs)+rowHeight(first,widths,fs)+6;
    if(y-needed<bottomLimit) newPage();
    rectFill(M,y-titleH,contentW,titleH,0.91); rectStroke(M,y-titleH,contentW,titleH);
    text(M+8,y-13,title,8.2,true); y-=titleH+gap;
    table(headers,rows,widths,opts);
  };

  text(M,y,'Summary & Detail',13,true); y-=15;
  text(M,y,`Date From: ${report.fromDate}    To: ${report.toDate}`,7.5,false); y-=10;

  const mrows=[
    ['1','No of Target Trains',report.total.targetTrains,report.up.targetTrains,report.dn.targetTrains],
    ['2','TOTAL NO. OF TRAIN RUNNING WITH KAVACH',report.total.runningWithKavach,report.up.runningWithKavach,report.dn.runningWithKavach],
    ['3','TOTAL NO. OF TRAIN RUNNING WITH KAVACH IN WORKING CONDITION',report.total.workingCondition,report.up.workingCondition,report.dn.workingCondition],
    ['4','NON KAVACH LOCO',report.total.nonKavach,report.up.nonKavach,report.dn.nonKavach],
    ['5','KAVACH DEFECTIVE LOCO',report.total.defective,report.up.defective,report.dn.defective],
    ['6','OTHER MAKE',report.total.otherMake,report.up.otherMake,report.dn.otherMake],
    ['7','OTHER REASON',report.total.otherReason,report.up.otherReason,report.dn.otherReason]
  ];
  table(['','DETAILS','TOTAL','UP','DN'],mrows,[18,295,75,75,75],{alignCenter:true,fontSize:7.1,lineH:8.1});

  const divisionRows=(rows,showStation=false)=>rows.length?rows.map((r,i)=>[i+1,showStation?(r.station||r.name):r.name,r.total,r.up,r.dn]):[[1,'No data',0,0,0]];
  section('KAVACH DEFECTIVE LOCO',['','SHED','TOTAL','UP','DN'],divisionRows(report.defectiveByShed),[18,295,75,75,75],{alignCenter:true,fontSize:7.1});
  section('NON KAVACH LOCO (MISLINK BY LOCO LINK DIVISION)',['','LOCO LINK DIVISION','TOTAL','UP','DN'],divisionRows(report.nonKavachLinkDivision),[18,295,75,75,75],{alignCenter:true,fontSize:7.1});
  section('NON KAVACH LOCO (MISLINK BY LOCO ATTACH DIVISION)',['','LOCO ATTACH DIVISION','TOTAL','UP','DN'],divisionRows(report.nonKavachAttachedDivision),[18,295,75,75,75],{alignCenter:true,fontSize:7.1});
  section('NON KAVACH LOCO',['','Station','TOTAL','UP','DN'],divisionRows(report.nonKavachPlain,true),[18,295,75,75,75],{alignCenter:true,fontSize:7.1});

  const detailTable=(title,rows,includeDate)=>{
    const heads=includeDate?['S.N','Date','Train No','Loco No','SHED','KAVACH MAKE','TLC Remark','Shed Remark','OEM Remark']:['S.N','Train No','Loco No','SHED','KAVACH MAKE','TLC Remark','Shed Remark','OEM Remark'];
    const rr=rows.length?rows.map(r=>includeDate?[r.sn,r.date,r.trainNo,r.locoNo,r.shed,r.kavachMake,r.tlcRemark,r.shedRemark,r.oemRemark]:[r.sn,r.trainNo,r.locoNo,r.shed,r.kavachMake,r.tlcRemark,r.shedRemark,r.oemRemark]):[includeDate?[1,'No data','','','','','','','']:[1,'No data','','','','','','']];
    const widths=includeDate?[20,42,48,50,40,60,92,93,93]:[20,50,55,45,65,100,100,103];
    section(title,heads,rr,widths,{alignCenter:true,alignAll:true,fontSize:5.7,lineH:7.0,pad:2.5});
  };
  detailTable('KAVACH DEFECTIVE LOCO DETAILS',report.defectiveRows,true);
  detailTable('OTHER REASON',report.otherRows,true);
...
app.get('/api/summary-detail/records', async (req,res)=>{
  try {
    const {fromDate,toDate,dir,metric,groupValue}=req.query;
    ...
    const all=await SavedRow.find({}).sort({searchDate:1,sourceRow:1}).lean();
    const base=filterSaved(all,fromDate,toDate,dir||'ALL');
    const rows=detailMetricRecords(base,String(metric||'totalTrains'),dir||'',groupValue||'');
    res.json({fromDate,toDate,dir:String(dir||'ALL').toUpperCase(),metric,metricLabel:SUMMARY_METRIC_LABELS[metric]||metric,total:rows.length,rows});
  } catch(e){res.status(500).json({message:e.message});}
});

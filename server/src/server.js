...
function detailSummaryForRecords(records) {
  ...
  // The MISLINK BY LOCO LINK DIVISION section must be driven ONLY by the
  // exact `If No - Reason` dropdown value used for MISLINK LOCO. Do not infer
  // this section from arbitrary text containing the word MISLINK, otherwise
  // records with another reason can incorrectly appear in the report.
  const isMislinkLoco = x => reason(x) === 'MISLINK LOCO';
  const divisionRows = (field) => {
    const map = new Map();
    records.filter(x => isMislinkLoco(x)).forEach(x => {
...
function createSummaryDetailPdf(report){
  ...
  const division=(title,rows)=>{ensure(22);text(M,y,title,8,true);y-=11;const rr=rows.length?rows.map((r,i)=>[i+1,r.name,r.total,r.up,r.dn]):[[1,'—',0,0,0]];table(['','', 'TOTAL','UP','DN'],rr,[18,270,75,75,75],{alignCenter:true,fontSize:7});};

...
function detailSummaryForRecords(records) {
  ...
  // Both NON KAVACH / MISLINK division sections are driven from the saved
  // Dashboard data. If `If No - Reason` contains any value, use the related
  // division field from that same record. Do not hard-code a single reason
  // such as `MISLINK LOCO`; otherwise valid records with another populated
  // dropdown value are incorrectly omitted from the report.
  const divisionRows = (field) => {
    const map = new Map();
    records.filter(x => reason(x) && String(x.row?.[field] || '').trim()).forEach(x => {

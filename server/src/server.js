...
function detailSummaryForRecords(records) {
  ...
  // Summary & Detail is a report of ALREADY SAVED dashboard records only.
  // Keep a small compatibility mapper so records saved by older builds/export
  // shapes are still read from the same SavedRow document. No source Excel
  // workbook is consulted for these MISLINK sections.
  ...
  // IMPORTANT: Summary & Detail must use ONLY the already-saved Dashboard
  // records (the same SavedRow records shown by Saved Data / Export). Never
  // read the source workbook for these sections.
  ...
  // IMPORTANT: The LOCO LINK MISLINK section is NOT a generic division
  // report. It must be populated only when the saved Dashboard record's
  // `If No - Reason` is explicitly the MISLINK option. Do not infer a
  // MISLINK-LINK record merely because LOCO_LINK_DIV contains a value.
  // The current TRAINS_PER_DAY dropdown uses the exact value `MISLINK LOCO`.
  // A future dedicated label is accepted only when it is explicitly selected.
  const isMislinkLocoLink = x => {
    const r = reason(x);
    return r === 'MISLINK LOCO' || r === 'MISLINK BY LOCO LINK DIVISION';
  };
  const isMislink = x => /MISLINK/.test(reason(x));
  const divisionRows = (getDivision, predicate = isMislink) => {
    const map = new Map();
    records.filter(predicate).forEach(x => {
  ...
  return { total:section(records), up:section(up), dn:section(dn), defectiveRows, defectiveByShed:[...shedMap.values()].sort((a,b)=>b.total-a.total), nonKavachLinkDivision:divisionRows(locoLinkDivision, isMislinkLocoLink), nonKavachAttachedDivision:divisionRows(locoAttachDivision, isMislink), otherRows };
}
...
function createSummaryDetailPdf(report){
  ...
  division('KAVACH DEFECTIVE LOCO',report.defectiveByShed); division('NON KAVACH LOCO (MISLINK BY LOCO LINK DIVISION)',report.nonKavachLinkDivision); division('NON KAVACH LOCO (MISLINK BY LOCO ATTACH DIVISION)',report.nonKavachAttachedDivision);

...
function detailSummaryForRecords(records) {
  ...
  // Summary & Detail uses ONLY existing SavedRow records (the same records
  // shown by Dashboard / Saved Data / Export). `searchDate` is the stored
  // Date run Kota div value for these records. No source workbook is read.
  ...
  // `If No - Reason` is the SOLE classification field for the three
  // NON KAVACH LOCO categories. Keep matching exact after trimming/case
  // normalization; do not infer a category from any division field.
  const reason = (x) => savedValue(x, [
    'noReason',
    'If_No_the_Reason',
    'If No - Reason',
    'If_No_the_Reason \n( Non Kavach Loco\n/ Kavach defective\n/ KAVACH Fitness cirtification N.Avl.\n/ Crew incompetency)'
  ...
  const NON_KAVACH_CATEGORIES = Object.freeze({
    ...
    NON_KAVACH: 'NON KAVACH LOCO'
  });
  const isCategory = (x, category) => reason(x) === category;
  const isNonKavachCategory = (x) => Object.values(NON_KAVACH_CATEGORIES).includes(reason(x));

  const locoLinkDivision = (x) => savedValue(x, [
    'locoLinkDiv',
    'LOCO_LINK_DIV',
    'LOCO_LINK_DIVISION',
    'Loco Link Division'
  ]);
  const locoAttachDivision = (x) => savedValue(x, [
    'attachedDiv',
    'LOCO_ATTACHED_DIV.',
    'LOCO_ATTACHED_DIV',
    'LOCO_ATTACHED_DIVISION',
    'Loco Attached Division'
  ]);
  ...
  const section = (arr) => {
    ...
    const nonKavachCount = count(arr, isNonKavachCategory);
    // As per the reference report, TOTAL NO. OF TRAIN RUNNING WITH KAVACH
    // is working + defective + other make + other reason.
    const runningWithKavach = workingCondition + defectiveCount + otherMakeCount + otherReasonCount;
    return {
      targetTrains: arr.length,
      runningWithKavach,
      workingCondition,
      nonKavach: nonKavachCount,
      defective: defectiveCount,
      otherMake: otherMakeCount,
      otherReason: otherReasonCount,
      nonKavachLink: count(arr, x => isCategory(x, NON_KAVACH_CATEGORIES.LINK)),
      nonKavachAttach: count(arr, x => isCategory(x, NON_KAVACH_CATEGORIES.ATTACH)),
      nonKavachPlain: count(arr, x => isCategory(x, NON_KAVACH_CATEGORIES.NON_KAVACH))
    };
  };
  ...
  // Each MISLINK division table is driven by its own exact If No - Reason
  // value. A division value by itself can NEVER make a record appear here.
  const divisionRows = (getDivision, category) => {
    const map = new Map();
    records.filter(x => isCategory(x, category)).forEach(x => {
      const value = getDivision(x);
      if (!value) return;
      if(!map.has(value)) map.set(value,{name:value,total:0,up:0,dn:0});
      const item=map.get(value);
      item.total++;
      const d=String(x.row?.dir||'').trim().toUpperCase();
      if(d==='UP') item.up++;
      if(d==='DN') item.dn++;
    });
  ...
  return {
    ...
    dn:section(dn),
    defectiveRows,
    defectiveByShed:[...shedMap.values()].sort((a,b)=>b.total-a.total),
    nonKavachLinkDivision:divisionRows(locoLinkDivision, NON_KAVACH_CATEGORIES.LINK),
    nonKavachAttachedDivision:divisionRows(locoAttachDivision, NON_KAVACH_CATEGORIES.ATTACH),
    // This category has no division field in the requirement. Keep the same
    // TOTAL/UP/DN table shape without inventing a grouping field.
    nonKavachPlain: (() => {
      const rows = records.filter(x => isCategory(x, NON_KAVACH_CATEGORIES.NON_KAVACH));
      return rows.length ? [{
        name: 'NON KAVACH LOCO',
        total: rows.length,
        up: rows.filter(x => String(x.row?.dir || '').trim().toUpperCase() === 'UP').length,
        dn: rows.filter(x => String(x.row?.dir || '').trim().toUpperCase() === 'DN').length
      }] : [];
    })(),
    otherRows
  };
...
function createSummaryDetailPdf(report){
  ...
  const table=(headers, rows, widths, opts={})=>{
    ...
    const drawRow=(cells,isHeader=false)=>{
      ...
      const rh=Math.max(1,...wraps.map(a=>a.length))*lineH+pad*2; ensure(rh);
      let x=M; for(let i=0;i<widths.length;i++){ line(x,y,x,y-rh); line(x+widths[i],y,x+widths[i],y-rh); if(isHeader&&headerBg){ops.push('0.94 g');ops.push(`${x.toFixed(2)} ${(y-rh).toFixed(2)} ${widths[i].toFixed(2)} ${rh.toFixed(2)} re f`);ops.push('0 g');line(x,y,x,y-rh);line(x+widths[i],y,x+widths[i],y-rh);} for(let j=0;j<wraps[i].length;j++){const tx=opts.alignCenter&&i>0?x+widths[i]/2: x+pad; const align=opts.alignCenter&&i>0?'center':'left'; if(align==='center'){const est=wraps[i][j].length*fs*0.48;text(tx-est/2,y-pad-fs-j*lineH,wraps[i][j],fs,isHeader)} else text(tx,y-pad-fs-j*lineH,wraps[i][j],fs,isHeader)} x+=widths[i]; } line(M,y-rh,M+widths.reduce((a,b)=>a+b,0),y-rh); y-=rh;
    };
  ...
  table(['','DETAILS','TOTAL','UP','DN'],mrows,[18,270,75,75,75],{alignCenter:true,fontSize:7});
  const division=(title,rows)=>{ensure(22);text(M,y,title,8,true);y-=11;const rr=rows.length?rows.map((r,i)=>[i+1,r.name,r.total,r.up,r.dn]):[[1,'No data',0,0,0]];table(['','', 'TOTAL','UP','DN'],rr,[18,270,75,75,75],{alignCenter:true,fontSize:7});};
  division('KAVACH DEFECTIVE LOCO',report.defectiveByShed); division('NON KAVACH LOCO (MISLINK BY LOCO LINK DIVISION)',report.nonKavachLinkDivision); division('NON KAVACH LOCO (MISLINK BY LOCO ATTACH DIVISION)',report.nonKavachAttachedDivision); division('NON KAVACH LOCO',report.nonKavachPlain);
  ...
  detail('KAVACH DEFECTIVE LOCO DETAILS',report.defectiveRows,true); detail('OTHER REASON',report.otherRows,false);

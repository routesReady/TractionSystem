...
function detailSummaryForRecords(records) {
  ...
  const reason = (x) => savedValue(x, ['noReason', 'If_No_the_Reason', 'If No - Reason']).toUpperCase();
  const locoLinkDivision = (x) => savedValue(x, ['locoLinkDiv', 'LOCO_LINK_DIV']);
  const locoAttachDivision = (x) => savedValue(x, ['attachedDiv', 'LOCO_ATTACHED_DIV.']);
  ...
  // `If No - Reason = MISLINK LOCO` identifies a MISLINK record; the division
  // value is then read from that exact same saved record. This prevents other
  // reasons such as KAVACH DEFECTIVE from incorrectly appearing as MISLINK.
  const isMislink = x => reason(x) === 'MISLINK LOCO';

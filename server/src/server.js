...
function detailSummaryForRecords(records) {
  ...
  // The two MISLINK sections must come ONLY from records that have already
  // been filled/saved from the Dashboard and are visible in Saved Data / Export.
  // A record is a MISLINK record when its saved `If No - Reason` value contains
  // MISLINK. The division is then taken from that exact same saved record.
  // This supports the current dropdown value `MISLINK LOCO` as well as future
  // MISLINK-specific dropdown labels, without ever reading the source Excel.
  const isMislink = x => /MISLINK/.test(reason(x));
  const divisionRows = (getDivision) => {
    const map = new Map();
    records.filter(x => isMislink(x)).forEach(x => {
  ...
  return { total:section(records), up:section(up), dn:section(dn), defectiveRows, defectiveByShed:[...shedMap.values()].sort((a,b)=>b.total-a.total), nonKavachLinkDivision:divisionRows(locoLinkDivision), nonKavachAttachedDivision:divisionRows(locoAttachDivision), otherRows };
}

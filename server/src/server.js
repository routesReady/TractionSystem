...
function filterSaved(all, startDate, endDate, dir = '', metric = '', groupValue='') {
  ...
  return metric ? metricRecords(base, metric, groupValue) : base;
}
...
function detailSummaryForRecords(records) {
  ...
  const section = arr => {
    ...
    return {
      ...
      nonKavachLink: count('nonKavachLink'),
      nonKavachAttach: count('nonKavachAttach'),
      nonKavachPlain: count('nonKavachPlain')
    };
  };
  ...
  const divisionRows = (getDivision, wantedReason) => {
    const map = new Map();
    records.filter(x => savedReason(x) === wantedReason).forEach(x => {
  ...
  metricRecords(records, 'defective').forEach(x=>{const name=String(x.row?.shed||'—').trim()||'—';if(!shedMap.has(name))shedMap.set(name,{name,total:0,up:0,dn:0});const item=shedMap.get(name);item.total++;if(savedDir(x)==='UP')item.up++;if(savedDir(x)==='DN')item.dn++;});
...
function buildPdfDocument(pages) {
  ...
  // A4 landscape is intentional: the detailed report has nine columns and long
  // TLC/Shed/OEM remarks. Landscape gives those fields enough physical width.
...
function createSummaryDetailPdf(report){
  // Landscape A4 keeps all nine detail columns readable. The previous portrait
  // renderer had too little width for long remarks, which caused clipping and
  // visual collisions even when the row height was increased.
  const W=841.89,H=595.28,M=25,contentW=W-M*2,pages=[],ops=[];
  const headerH=47,footerH=20,bottomLimit=M+footerH+5;
  let y=H-M-headerH,pageNo=1;

  const text=(x,yy,str,size=7,bold=false)=>{
    const font=bold?'/F2':'/F1';
    ops.push(`BT ${font} ${size} Tf ${x.toFixed(2)} ${yy.toFixed(2)} Td (${pdfEscape(pdfAscii(str))}) Tj ET`);
  };
  ...
  const drawPageHeader=()=>{
    rectFill(M,H-M-31,contentW,31,0.94);
    rectStroke(M,H-M-31,contentW,31);
    text(M+9,H-M-13,'TRACTION OPERATION - KAVACH MONITORING SYSTEM',10,true);
    text(M+9,H-M-25,'West Central Railway, Kota Division',7.2,false);
    const dateLabel=`Summary & Detail | ${report.fromDate} to ${report.toDate}`;
    const dateX=Math.max(M+330,W-M-9-(dateLabel.length*3.4));
    text(dateX,H-M-13,dateLabel,7,false);
    line(M,H-M-34,W-M,H-M-34);
  };
  const drawPageFooter=()=>{
    line(M,M+13,W-M,M+13);
    text(M,M+4,'Traction Operation - Kavach Monitoring System | Kota Division',6.2,false);
    text(W-M-48,M+4,`Page ${pageNo}`,6.2,false);
  };
  ...
  const wrapFor=(value,width,fs,pad=3)=>wrapPdfText(value,Math.max(2,Math.floor((width-pad*2)/(fs*0.56))));
  const rowHeightFromLines=(lineCount,lineH,pad)=>Math.max(1,lineCount)*lineH+pad*2;
  const maxLinesAvailable=(lineH,pad)=>Math.max(1,Math.floor((y-bottomLimit-pad*2)/lineH));

  const table=(headers,rows,widths,opts={})=>{
    const fs=opts.fontSize||6.8,pad=opts.pad||3,lineH=opts.lineH||8.2,totalW=widths.reduce((a,b)=>a+b,0);
    let rowIndex=0;

    const drawChunk=(cells,wraps,startLine,count,isHeader=false)=>{
      const rh=rowHeightFromLines(count,lineH,pad);
      if(y-rh<bottomLimit) return false;
      const rowBg=isHeader?0.88:(rowIndex%2===0?0.97:1);
      if(rowBg<1) rectFill(M,y-rh,totalW,rh,rowBg);
      let x=M;
      for(let i=0;i<widths.length;i++){
        line(x,y,x,y-rh); line(x+widths[i],y,x+widths[i],y-rh);
        const vals=wraps[i].slice(startLine,startLine+count);
        for(let j=0;j<vals.length;j++){
          const value=vals[j];
          const centered=!isHeader && opts.leftBodyColumns?.includes(i) ? false : Boolean(opts.alignCenter && (opts.alignAll || i>0));
          const tx=centered ? x+widths[i]/2-(value.length*fs*0.235) : x+pad;
          text(Math.max(x+1,Math.min(tx,x+widths[i]-pad-1)),y-pad-fs-j*lineH,value,fs,isHeader);
        }
        x+=widths[i];
      }
      line(M,y-rh,M+totalW,y-rh);
      y-=rh;
      return true;
    };

    const drawHeader=()=>{
      const wraps=headers.map((c,i)=>wrapFor(c,widths[i],fs,pad));
      const lines=Math.max(1,...wraps.map(a=>a.length));
      if(y-rowHeightFromLines(lines,lineH,pad)<bottomLimit) newPage();
      drawChunk(headers,wraps,0,lines,true);
    };
    ...
    for(const row of rows){
      ...
      while(startLine<totalLines){
        const available=maxLinesAvailable(lineH,pad);
        if(available<1){newPage();drawHeader();continue;}
        const count=Math.min(available,totalLines-startLine);
        if(!drawChunk(row,wraps,startLine,count,false)){ newPage(); drawHeader(); continue; }
        startLine+=count;
        if(startLine<totalLines){newPage();drawHeader();}
      }
      rowIndex++;
    }
    y-=6;
  ...
  const section=(title,headers,rows,widths,opts={})=>{
    const fs=opts.fontSize||6.8,pad=opts.pad||3,lineH=opts.lineH||8.2;
    const first=rows.length?rows[0]:headers.map(()=> '');
    const headerLines=Math.max(1,...headers.map((c,i)=>wrapFor(c,widths[i],fs,pad).length));
    const firstLines=Math.max(1,...first.map((c,i)=>wrapFor(c,widths[i],fs,pad).length));
    const needed=22+6+rowHeightFromLines(headerLines,lineH,pad)+Math.min(firstLines,Math.max(1,Math.floor((y-bottomLimit-pad*2)/lineH))) * lineH + pad*2 + 4;
    if(y-needed<bottomLimit) newPage();
    rectFill(M,y-19,contentW,19,0.90); rectStroke(M,y-19,contentW,19);
    text(M+8,y-13,title,8.1,true);
    y-=25;
  ...
  table(['','DETAILS','TOTAL','UP','DN'],mrows,[24,450,105,105,105],{alignCenter:true,fontSize:7.3,lineH:8.6,pad:3});
  ...
  section('KAVACH DEFECTIVE LOCO',['','SHED','TOTAL','UP','DN'],divisionRows(report.defectiveByShed),[24,450,105,105,105],{alignCenter:true,fontSize:7.2,lineH:8.4,pad:3});
  section('NON KAVACH LOCO (MISLINK BY LOCO LINK DIVISION)',['','LOCO LINK DIVISION','TOTAL','UP','DN'],divisionRows(report.nonKavachLinkDivision),[24,450,105,105,105],{alignCenter:true,fontSize:7.2,lineH:8.4,pad:3});
  section('NON KAVACH LOCO (MISLINK BY LOCO ATTACH DIVISION)',['','LOCO ATTACH DIVISION','TOTAL','UP','DN'],divisionRows(report.nonKavachAttachedDivision),[24,450,105,105,105],{alignCenter:true,fontSize:7.2,lineH:8.4,pad:3});
  section('NON KAVACH LOCO',['','STATION','TOTAL','UP','DN'],divisionRows(report.nonKavachPlain,true),[24,450,105,105,105],{alignCenter:true,fontSize:7.2,lineH:8.4,pad:3});

  const detailTable=(title,rows,includeDate)=>{
    const heads=includeDate?['S.N','Date','Train No','Loco No','SHED','KAVACH MAKE','TLC Remark','Shed Remark','OEM Remark']:['S.N','Date','Train No','Loco No','SHED','KAVACH MAKE','TLC Remark','Shed Remark','OEM Remark'];
    const rr=rows.length?rows.map(r=>[r.sn,r.date,r.trainNo,r.locoNo,r.shed,r.kavachMake,r.tlcRemark,r.shedRemark,r.oemRemark]):[[1,'No data','','','','','','','']];
    // The three remark columns get most of the landscape width. Text is wrapped
    // and a single logical record may continue onto another page if necessary.
    const widths=[20,50,58,58,42,64,166,166,166];
    section(title,heads,rr,widths,{alignCenter:true,alignAll:true,leftBodyColumns:[6,7,8],fontSize:5.8,lineH:7.4,pad:3});
  };
...
app.get('/api/summary/metric-export', async (req,res)=>{
  try {
    ...
    if(!validDate(fromDate||'') || !validDate(toDate||'')) return res.status(400).json({message:'From Date and To Date must be DD-MM-YYYY.'});
    if(parseDMY(fromDate)>parseDMY(toDate)) return res.status(400).json({message:'From Date cannot be after To Date.'});
    if(!SUMMARY_METRICS.has(String(metric||'')) && !['defectiveByShed'].includes(String(metric||''))) return res.status(400).json({message:'Unknown summary metric.'});

  ...
  const syncRole = async (names, role) => {
    ...
    for (const username of names) {
      await User.findOneAndUpdate(
        { username, role },
        { $setOnInsert: { username, role, passwordHash: hash, isActive: true, mustChangePassword: true, authVersion: 0 } },
...
app.get('/api/remark-records', userAuth, requireRole('shed','oem'), async (req, res) => {
  try {
    ...
    const rows = all.filter(x => {
      ...
      return identity && (worked==='NO' || worked==='PARTIALLY WORKING') && allowedReasons.has(reason);
    });
...
app.patch('/api/remark-records/:id', userAuth, requireRole('shed','oem'), async (req,res) => {
  try {
    ...
    if(isShed){ row.shedRemark=value; record.shedRemark=value; } else { row.oem=value; record.oem=value; }
    record.row=row;
    await record.save();
    res.json({message:isShed?'Shed remark updated successfully.':'OEM remark updated successfully.', row:record});
  }catch(e){res.status(500).json({message:e.message});}
});
...
app.get('/api/data/saved', userAuth, requireRole('admin'), async (req, res) => {
  try {
...
app.get('/api/data/export', userAuth, requireRole('admin'), async (req, res) => {
  try {
...
app.get('/api/summary', userAuth, requireRole('admin'), async (req, res) => {
  try {
...
app.get('/api/summary/export', userAuth, requireRole('admin'), async (req, res) => {
  try {
    const { fromDate, toDate, dir } = req.query;
    if (!validDate(fromDate || '') || !validDate(toDate || '')) return res.status(400).json({ message: 'From Date and To Date must be DD-MM-YYYY.' });
    if (parseDMY(fromDate) > parseDMY(toDate)) return res.status(400).json({ message: 'From Date cannot be after To Date.' });

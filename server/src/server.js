...
function detailSummaryForRecords(records) {
  ...
  const defective = x => ['KERNEX APP. PENDING','KAVACH DO NOT START','KAVACH ISOLATION PREV.'].includes(reason(x));
...
function detailMetricRecords(records, metric, dir='', groupValue='') {
  ...
  switch (metric) {
    ...
    case 'defective': rows = rows.filter(x => ['KERNEX APP. PENDING','KAVACH DO NOT START','KAVACH ISOLATION PREV.'].includes(reason(x))); break;
    ...
    case 'defectiveByShed': rows = rows.filter(x => ['KERNEX APP. PENDING','KAVACH DO NOT START','KAVACH ISOLATION PREV.'].includes(reason(x)) && String(x.row?.shed||'').trim()===String(groupValue||'').trim()); break;
    default: break;
  }
  return rows;
}
...
function createSummaryDetailPdf(report){
  const W=595.28,H=841.89,M=28.35, contentW=W-M*2, pages=[], ops=[]; let y=H-M;
  const newPage=()=>{if(ops.length) pages.push(ops.splice(0,ops.length)); y=H-M;};
  const text=(x,yy,str,size=7,bold=false)=>{ops.push(`BT /F1 ${size} Tf ${x.toFixed(2)} ${yy.toFixed(2)} Td (${pdfEscape(pdfAscii(str))}) Tj ET`)};
  const line=(x1,y1,x2,y2)=>ops.push(`${x1.toFixed(2)} ${y1.toFixed(2)} m ${x2.toFixed(2)} ${y2.toFixed(2)} l S`);
  const ensure=h=>{if(y-h< M+8)newPage();};
  const table=(headers, rows, widths, opts={})=>{
    const fs=opts.fontSize||6.3, pad=3, lineH=7.4, headerBg=opts.headerBg!==false;
    const drawRow=(cells,isHeader=false)=>{
      const wraps=cells.map((c,i)=>wrapPdfText(c,Math.max(3,Math.floor((widths[i]-pad*2)/(fs*0.52)))));
      ...
      for(let i=0;i<widths.length;i++){
        if(isHeader&&headerBg){ops.push('0.94 g');ops.push(`${x.toFixed(2)} ${(y-rh).toFixed(2)} ${widths[i].toFixed(2)} ${rh.toFixed(2)} re f`);ops.push('0 g');}
        line(x,y,x,y-rh); line(x+widths[i],y,x+widths[i],y-rh);
        for(let j=0;j<wraps[i].length;j++){
          const align=opts.alignCenter&&i>0?'center':'left';
          const tx=align==='center'?x+widths[i]/2:x+pad;
          if(align==='center'){const est=wraps[i][j].length*fs*0.48;text(tx-est/2,y-pad-fs-j*lineH,wraps[i][j],fs,isHeader)}
          else text(tx,y-pad-fs-j*lineH,wraps[i][j],fs,isHeader);
        }
        x+=widths[i];
      }
      line(M,y-rh,M+widths.reduce((a,b)=>a+b,0),y-rh); y-=rh;
    };
    drawRow(headers,true); for(const r of rows) drawRow(r,false); y-=4;
  };
  text(M,y,'Summary & Detail - Date From '+report.fromDate+' To '+report.toDate,10,true); y-=16;
  const mrows=[
  ...
  table(['','DETAILS','TOTAL','UP','DN'],mrows,[18,295,75,75,75],{alignCenter:true,fontSize:7});
  const division=(title,rows,showStation=false)=>{ensure(24);text(M,y,title,8,true);y-=12;const rr=rows.length?rows.map((r,i)=>[i+1,showStation?(r.station||r.name):r.name,r.total,r.up,r.dn]):[[1,'No data',0,0,0]];table(['',showStation?'Station':'', 'TOTAL','UP','DN'],rr,[18,295,75,75,75],{alignCenter:true,fontSize:7});};
  division('KAVACH DEFECTIVE LOCO',report.defectiveByShed); division('NON KAVACH LOCO (MISLINK BY LOCO LINK DIVISION)',report.nonKavachLinkDivision); division('NON KAVACH LOCO (MISLINK BY LOCO ATTACH DIVISION)',report.nonKavachAttachedDivision); division('NON KAVACH LOCO',report.nonKavachPlain,true);
  const detail=(title,rows,includeDate)=>{ensure(22);text(M,y,title,8,true);y-=11;const heads=includeDate?['S.N','Date','Train No','Loco No','SHED','KAVACH MAKE','TLC Remark','Shed Remark','OEM Remark']:['S.N','Train No','Loco No','SHED','KAVACH MAKE','TLC Remark','Shed Remark','OEM Remark'];const rr=rows.length?rows.map(r=>includeDate?[r.sn,r.date,r.trainNo,r.locoNo,r.shed,r.kavachMake,r.tlcRemark,r.shedRemark,r.oemRemark]:[r.sn,r.trainNo,r.locoNo,r.shed,r.kavachMake,r.tlcRemark,r.shedRemark,r.oemRemark]):[includeDate?[1,'','','','','','','','']:[1,'','','','','','','']];const widths=includeDate?[20,42,48,50,40,60,92,93,93]:[20,50,55,45,65,100,100,103]; table(heads,rr,widths,{alignCenter:true,fontSize:6});};
  detail('KAVACH DEFECTIVE LOCO DETAILS',report.defectiveRows,true); detail('OTHER REASON',report.otherRows,true);
  if(ops.length) pages.push(ops); return buildPdfDocument(pages);
}

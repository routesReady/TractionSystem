...
function NonKavachAttachDetailTable({rows=[]}){
 const headers=['S.No.','Date','Train No','Loco No','SHED','Loco Attach Div','TLC Remark'];
 return <section className="excel-detail-section records-report-section non-kavach-detail-section"><h4>NON KAVACH LOCO ATTACH BY ATTACH DIVISON</h4><div className="detail-record-scroll"><table className="excel-detail-table record-report-table non-kavach-attach-table"><thead><tr>{headers.map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{rows.length?rows.map(r=><tr key={`non-kavach-${r.sn}`}><td>{r.sn}</td><td>{r.date||''}</td><td>{r.trainNo||''}</td><td>{r.locoNo||''}</td><td>{r.shed||''}</td><td>{r.locoAttachDiv||''}</td><td className="remark-cell">{r.tlcRemark||''}</td></tr>):<tr><td>1</td><td colSpan={6}>No data</td></tr>}</tbody></table></div></section>
}
